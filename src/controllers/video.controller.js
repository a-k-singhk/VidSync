...
const uploadVideo=asyncHandler(async(req,res)=>{
    ...
    const existTitle=Video.findOne({title});
    if(existTitle){
        throw new ApiError(409,"Title already exist in your video, please change the title");
    }
    //input validation
    if(!title || !description){
        throw new ApiError(401,"Title or description does not exist");
    }
    if(!req.files || !req.files.videoFile || !req.files.thumbnail){
        throw new ApiError(401,"files for thumbnail does not exist");
    }
    const videoFileLocalPath=req.files.videoFile[0].path;
    const thumbnailFileLocalPath=req.files.thumbnail[0].path;

    //Uplaod Video file to cloudinary
...
export {
    getAllVideo, uploadVideo, updateVideo, deleteVideo 
}